...
export default function ItemDetails(props){
    ...
    const nftdata = data.find((jobs) => jobs.id === parseInt (props.params.id));
    
    const BidItem =[
        {
            image:'/images/items/2.gif',
            title:'2 WETH',
            title1:"0xe849fa28a...ea14",
            hours:'6 hours ago'
        },
        {
                ...
                <div className="container-fluid relative px-3">
                    <div className="layout-specing">
                        <div className="md:flex justify-between items-center">
                            <h5 className="text-lg font-semibold">{nftdata?.title ? nftdata?.title : "Probably A Label #3277" }</h5>
                            ...
                            <ul className="breadcrumb tracking-[0.5px] mb-0 inline-block mt-1 md:mt-0">
                                ...
                                <li className="inline breadcrumb-item text-[15px] font-semibold duration-500 text-slate-900 dark:text-white" aria-current="page">Item Detail</li>
                            </ul>
                        </div>

                        <div className="grid lg:grid-cols-12 md:grid-cols-2 grid-cols-1 gap-6 mt-6">
                            <div className="lg:col-span-5">
                                <Image src={ nftdata?.image } width={0} height={0} sizes="100vw" placeholder="blur" blurDataURL={nftdata?.image} className="rounded-md shadow dark:shadow-gray-700 w-full h-auto" alt=""/>
        
                                <div className="bg-white dark:bg-slate-900 rounded-md shadow dark:shadow-gray-800 mt-[30px] p-6">
                                    <div>
                            ...
                            <div className="lg:col-span-7">
                                ...
                                <div className="mt-6 flex space-x-1">
                                    ...
                                    <Link href="#" onClick={() => setBuyNow(!buyNow)} className="btn rounded-full bg-violet-600 hover:bg-violet-700 border-violet-600 hover:border-violet-700 text-white"><i className="mdi mdi-lightning-bolt"></i> Buy Now</Link>
                                </div>
                                <div className="md:flex p-6 bg-white dark:bg-slate-900 rounded-lg shadow dark:shadow-gray-700 mt-6">
                                    <div className="md:w-1/2">
                                        <div className="flex items-center">
                                            <div className="relative inline-block">
                                                <Image src={nftdata?.avatar } width={64} height={64} sizes="100vw" placeholder="blur" blurDataURL={nftdata?.avatar}  className="h-16 rounded-md" alt=""/>
                                                <i className="mdi mdi-check-decagram text-emerald-600 text-lg absolute -top-2 -end-2"></i>
                                            </div>
        
                                            <div className="ms-3">
                                                <Link href="/creator-profile" className="font-semibold block hover:text-violet-600">{nftdata?.name}</Link>
                                ...
                                <div className="grid grid-cols-1 mt-8">
                                    <ul className="md:w-fit w-full flex-wrap justify-center text-center p-3 bg-white dark:bg-slate-900 shadow dark:shadow-gray-800 rounded-md" id="myTab" data-tabs-toggle="#StarterContent" role="tablist">
                                        <li role="presentation" className="md:inline-block block md:w-fit w-full">
                                            <button className={`px-6 py-2 font-semibold rounded-md w-full transition-all duration-500 ease-in-out ${activeIndex === 0 ? "bg-violet-600" :""}`} onClick={()=>setActiveIndex(0)}>Bids</button>
                                        </li>
                                        <li role="presentation" className="md:inline-block block md:w-fit w-full">
                                            <button className={`px-6 py-2 font-semibold rounded-md w-full transition-all duration-500 ease-in-out  ${activeIndex === 1 ? "bg-violet-600" :""}`} onClick={()=>setActiveIndex(1)}>Activity</button>
                                        </li>
                                    </ul>
                                    ...
                                    {activeIndex === 0 ?
                                        ...
                                        <div>
                                            <div className="grid grid-cols-1">
                                                {BidItem.map((item,index)=>{
                                                    return(
                                                        <div className="flex items-center py-2" key={index}>
                                                            <div className="relative inline-block">
                                                                <Image src={item.image} width={64} height={64} placeholder="blur" blurDataURL={item.image} className="h-16 rounded-md" alt=""/>
                                                                <i className="mdi mdi-check-decagram text-emerald-600 text-lg absolute -top-2 -end-2"></i>
                                                            </div>
                            
                                                            <div className="ms-3">
                                                                <h6 className="font-semibold">{item.title}<span className="text-slate-400">by</span> <Link href="#" className="hover:text-violet-600 duration-500 ease-in-out">{item.title1}</Link></h6>
                                                                <span className="text-slate-400 text-[16px]">{item.hours}</span>
                                                            </div>
                                                        </div>
                                                    )
                                                })}
                                    ...
                                    {activeIndex === 1 ?
                                        ...
                                        <div>
                                            <div className="grid grid-cols-1">
                                                <div className="flex items-center">
                                                    <div className="relative inline-block">
                                                        <Image src='/images/items/1.gif' width={64} height={64} placeholder="blur" blurDataURL={item.image} className="h-20 rounded-md" alt=""/>
                                                        <i className="mdi mdi-check-decagram text-emerald-600 text-lg absolute -top-2 -end-2"></i>
                                                    </div>
